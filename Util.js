...
function Socket(socketNO) {
	...
	self.Port = "";//"9999";
	self.ws_open = false;
  self.reconn_sec = 5*1000;// 0 = not do
  self.reconn_internal = 0;
  self.ping_sec = 15*1000;
  ...
  self.reconnect = function(){
    ...
    */
    if ( self.conn.readyState == WebSocket.CONNECTING ) {
      // console.log("reconnect start");
      self.conn.onopen = function(){console.log("reconnect onopen");this.close();};
      self.conn.onmessage = function(){console.log("reconnect onmessage");};
      self.conn.onerror=function(){console.log("reconnect onerror");};
      self.conn.onclose=function(){console.log("reconnect onclose");};
  ...
  self.ping = function(){
    ...
    */
    ...
    if(top.encodelog)console.log(Str);
		var trans = "";
		var rev = "";
		var getcode = "";
	...
	self.decode = function(getstr) {
		...
		if(top.decodelog)console.log(self.Port+" "+destr);
		return destr;
	}
